...
kubectl wait --for=condition=ready --timeout=300s pod -l app=backend -n medical-appointment-develop
kubectl wait --for=condition=ready --timeout=300s pod -l app=frontend -n medical-appointment-develop
kubectl wait --for=condition=ready --timeout=300s pod -l app=postgres -n medical-appointment-develop
```
...
# Check services
kubectl get svc -n medical-appointment-develop
...
kubectl rollout restart deployment/frontend -n medical-appointment-develop

# 4. Watch the rollout
kubectl rollout status deployment/frontend -n medical-appointment-develop
```
...
helm upgrade medical-appointment ./charts/medical-appointment \
  --namespace medical-appointment-develop \
  --values charts/medical-appointment/values-develop.yaml
```
...
# Check events
kubectl get events -n medical-appointment-develop --sort-by='.lastTimestamp'
```
...
kubectl get pods -n medical-appointment-develop -l app=postgres
...
kubectl describe ingress medical-appointment -n medical-appointment-develop
...
kubectl exec -it deployment/backend -n medical-appointment-develop -- /bin/bash
kubectl exec -it statefulset/postgres -n medical-appointment-develop -- /bin/bash

# Get resource usage
kubectl top pods -n medical-appointment-develop
kubectl top nodes

# Get Helm release status
helm status medical-appointment -n medical-appointment-develop
helm get values medical-appointment -n medical-appointment-develop
